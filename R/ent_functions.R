...
counts_since_last_switch_for_log_reg <- function(dat, subN){
  # count up how many selections since the last switch, this uses
  # the dataset generated by get_Sw 
  tmp <- dat %>% filter(sub == subN)
  
  tmp <- tmp %>% mutate(
    idx = row_number() - 1L,
    idx = idx - first(idx), .by = Sw) 
  tmp
}
...
get_p_context <- function(dat, subN){
  ...
  psC <- c(.5, .5) # the probability you are in the same context as the last one you saw (pdC will be 1 - this number). p=.5 reflects uninformed priors, 
  # 2 values as the first update comes at the end of trial 2. 
  # This is because we are coding the probability that trial t is the same context as t-1, so the first update comes at the end of trial 2, as we need
  # 2 observations to make a same or different response.
